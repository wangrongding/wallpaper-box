...
import { CheckCircle2, Download, FolderOpen, Link2, Loader2, Upload, Video } from 'lucide-react'
...
export default function LiveWallpaper() {
  ...
  async function handleOpenVideoDirectory() {
    const response = await ipcRenderer.invoke('open-video-wallpaper-directory')
    if (!response?.success) {
      toast.error(response?.message || '打开视频保存目录失败')
    }
  }
  ...
  const downloadStatusTitle = downloadProgress?.title || lastDownloadedVideo?.title || lastDownloadedVideo?.fileName || '正在获取视频信息'
  const hasDownloadStatus = Boolean(downloadProgress || lastDownloadedVideo)
    ...
    <div className='live-wallpaper-page h-full overflow-hidden'>
      <div className='grid h-full grid-cols-[420px_minmax(0,1fr)] gap-4'>
        <section className='flex min-h-0 flex-col rounded-[14px] border border-white/10 bg-[linear-gradient(160deg,rgba(8,14,24,0.98),rgba(10,18,30,0.95))] p-4 shadow-[0_24px_60px_rgba(2,6,23,0.2)]'>
          ...
          <div className='mb-4 rounded-[22px] border border-white/10 bg-black/20 p-4'>
            ...
            <div className='mt-4 grid gap-2'>
              <div className='rounded-2xl bg-white/[0.03] px-3 py-2 text-[12px] text-slate-400'>
          ...
          <div className='mt-auto space-y-3'>
            <div className='rounded-[22px] border border-white/10 bg-black/20 p-4'>
              ...
              <div className='space-y-2 text-[12px] leading-6 text-slate-300'>
                ...
                <button
                  type='button'
                  className='group flex w-full items-center justify-between gap-3 rounded-2xl border border-white/[0.08] bg-white/[0.035] px-3 py-2 text-left transition-all hover:border-sky-300/25 hover:bg-sky-400/10 hover:text-sky-50'
                  onClick={() => void handleOpenVideoDirectory()}
                >
                  <span className='flex min-w-0 items-center gap-2'>
                    <FolderOpen className='h-3.5 w-3.5 shrink-0 text-sky-200' />
                    <span className='min-w-0'>
                      <span className='block truncate text-[11px] text-slate-500 group-hover:text-slate-300'>~/wallpaper-box/videos</span>
                    </span>
                  </span>
                  <span className='shrink-0 rounded-full bg-white/[0.06] px-2 py-0.5 text-[10px] font-medium text-slate-300 group-hover:bg-sky-300/15 group-hover:text-sky-100'>
                    打开文件夹
                  </span>
                </button>
              </div>
        ...
        <section className='flex min-h-0 flex-col rounded-[14px] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.025))] p-4 shadow-[0_28px_70px_rgba(2,6,23,0.2)]'>
            ...
            {hasDownloadStatus && (
              <div className='min-w-0 flex-1 pt-1.5'>
                <div className='mb-3 mt-1 flex items-center gap-3'>
                  <p className='min-w-0 flex-1 truncate text-[13px] font-medium text-slate-100'>{downloadStatusTitle}</p>
                  <span className='shrink-0 rounded-full bg-sky-400/10 px-2 py-0.5 text-[11px] font-semibold tabular-nums text-sky-100'>
                    {progressPercent}%
                  </span>
                </div>

                <div className='h-4 overflow-hidden rounded-full border border-white/[0.06] bg-black/25'>
                  <div
          ...
          <div className='relative flex-1 overflow-hidden rounded-[28px] border border-white/10 bg-[radial-gradient(circle_at_18%_0%,rgba(56,189,248,0.14),transparent_28%),radial-gradient(circle_at_100%_12%,rgba(250,204,21,0.08),transparent_24%),linear-gradient(180deg,rgba(8,13,24,0.98),rgba(10,15,24,0.94))] shadow-[inset_0_1px_0_rgba(255,255,255,0.04),0_24px_60px_rgba(2,6,23,0.28)]'>
            ...
            <div className='relative flex h-full items-center justify-center'>
              {filePath ? (
                <div className='relative h-full w-full overflow-hidden rounded-[24px] border border-white/10 bg-black/35 shadow-[0_24px_60px_rgba(2,6,23,0.42)]'>
                  <video
                    key={`ambient-${previewToken}`}
                    className='pointer-events-none absolute inset-0 h-full w-full scale-110 object-cover opacity-45 blur-2xl saturate-150'
                    src={previewSrc}
                    autoPlay
                    loop
                    muted
                    ...
                    aria-hidden='true'
                  />
                  <video key={previewToken} className='relative z-10 h-full w-full object-contain' src={previewSrc} autoPlay loop muted playsInline />
